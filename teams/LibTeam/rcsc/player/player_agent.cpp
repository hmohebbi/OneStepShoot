...
//added by Hosein Mohebbi Reza Etemadi
    void analyzeTurn( const char * msg);
...
PlayerAgent::parse( const char * msg )
{
    ...
    while (1){
        if ( ! std::strncmp( msg, "(last_touched ", 14 ) )
        {
            M_impl->analyzeTurn(msg);
        } else
            break;
    }
...
//added by Hosein Mohebbi Reza Etemadi
void strnDelete(char* str, int n){
    int i;
    for(i=n; str[i]; i++){
        str[i-n] = str[i];
    }
...
//added by Hosein Mohebbi Reza Etemadi
void
PlayerAgent::Impl::analyzeTurn( const char * msg )
{
    char id[14];
    int side;
    if ( std::sscanf( msg, "(%14s %d ",
                      id, &side ) != 2 )
    {
        std::cerr << agent_.world().teamName() << ' '
                  << agent_.world().self().unum() << ": "
                  << "last_touched parse error in ["
                  << msg
                  << std::endl;
        dlog.addText( Logger::SENSOR,
                      __FILE__" (analyzeTurn) last_touched parse error [%s]", msg );
        return;
    }

    agent_.M_worldmodel.setAccurateLastKickerSide(SideID(side));
    char* newMsg = const_cast<char*&>(msg);
    if ( side == -1 )
        strnDelete(newMsg, 18);
    else
        strnDelete(newMsg, 17);
